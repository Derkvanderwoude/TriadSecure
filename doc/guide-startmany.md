...
Copy the masternode private key and correspondig collateral output transaction that holds the 1000 ARC.

The masternode private key may be an existing key from [Option 1](#option1), or a newly generated key from [Option 2](#option2). 

*Note: The masternode priviate key is **not** the same as a wallet private key. **Never** put your wallet private key in the masternode.conf file. That is almost equivalent to putting your 1000 ARC on the remote server and defeats the purpose of a hot/cold setup.*
...
[From the triad github repo](https://github.com/TriadCore/triad/blob/goldmine/doc/masternode_conf.md)
...
## What about the triad.conf file?

If you are using a `masternode.conf` file you no longer need the `triad.conf` file. The exception is if you need custom settings (_thanks oblox_). In that case you **must** remove `masternode=1` from local `triad.conf` file. This option should be used only to start local Hot masternode now.

## Update triad.conf on server

If you generated a new masternode private key, you will need to update the remote `triad.conf` files.

Shut down the daemon and then edit the file.

```nano .triadcore/triad.conf```

### Edit the masternodeprivkey
If you generated a new masternode private key, you will need to update the `masternodeprivkey` value in your remote `triad.conf` file.
...
and comparing with the official explorer at https://explorer.triad.org/
...
Search your Masternodes on https://triad.org/masternodes.html
