...
Triad Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 7878 for connections and services them.
...
However, in open source software such as Triad Core, where everyone uses
...
pay attention to for reviewers of Triad Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Triad Core uses tinyformat, which is type safe. Leave them out to avoid confusion
