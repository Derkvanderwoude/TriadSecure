...
{
    BOOST_FOREACH(CAmount d, vecSpySendDenominations)
...

    {
        ...
        BOOST_FOREACH(const MasterKeyMap::value_type& pMasterKey, mapMasterKeys)
        {
            ...
            if (!crypter.Decrypt(pMasterKey.second.vchCryptedKey, vMasterKey))
                continue; // try another goldmine key
...
// Recursively determine the rounds of a given input (How deep is the SpySend chain for a given input)
int CWallet::GetRealInputSpySendRounds(CTxIn txin, int nRounds) const
{
    ...
    if(wtx != NULL)
    {
        ...
        if (mdwi == mDenomWtxes.end()) {
            ...
            LogPrint("privatesend", "GetRealInputSpySendRounds INSERTING %s\n", hash.ToString());
        ...
        if (nout >= wtx->vout.size()) {
            ...
            LogPrint("privatesend", "GetRealInputSpySendRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout, -4);
            return -4;
        }
        ...
        BOOST_FOREACH(CTxIn txinNext, wtx->vin) {
            if (IsMine(txinNext)) {
                int n = GetRealInputSpySendRounds(txinNext, nRounds + 1);
...
int CWallet::GetInputSpySendRounds(CTxIn txin) const
{
    LOCK(cs_wallet);
    int realSpySendRounds = GetRealInputSpySendRounds(txin, 0);
    return realSpySendRounds > nSpySendRounds ? nSpySendRounds : realSpySendRounds;
}
...
CAmount CWalletTx::GetAnonymizedCredit(bool fUseCache) const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++)
    {
        ...
        const int nRounds = pwallet->GetInputSpySendRounds(txin);
        if(nRounds >= nSpySendRounds){
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            if (!MoneyRange(nCredit))
...
CAmount CWallet::GetAnonymizableBalance() const
{
    ...
    BOOST_FOREACH(CompactTallyItem& item, vecTally) {
        ...
        if(IsDenominatedAmount(item.nAmount) || item.nAmount >= vecSpySendDenominations.back() * 10)
            nTotal += item.nAmount;
    }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputSpySendRounds(vin);
                fTotal += (float)rounds;
                fCount += 1;
            }
        }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int nRounds = GetInputSpySendRounds(txin);
                nTotal += pcoin->vout[i].nValue * nRounds / nSpySendRounds;
            }
        }
    }
...
CAmount CWallet::GetNeedsToBeAnonymizedBalance(CAmount nMinBalance) const
{
    ...
    CAmount nNeedsToAnonymizeBalance = nSpySendAmount*COIN - nAnonymizedBalance;
...
bool less_then_denom (const COutput& out1, const COutput& out2)
{
    ...
    bool found1 = false;
    bool found2 = false;
    BOOST_FOREACH(CAmount d, vecSpySendDenominations) // loop through predefined denoms
    {
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType nCoinType, bool fUseInstantSend) const
{
    ...
    if (coinControl && coinControl->HasSelected() && !coinControl->fAllowOtherInputs)
    {
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            if(nCoinType == ONLY_DENOMINATED) {
                ...
                if(nRounds < nSpySendRounds) continue;
            }
    ...
    if(nCoinType == ONLY_DENOMINATED) {
        CAmount nSmallestDenom = vecSpySendDenominations.back();
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH(CAmount nDenom, vecSpySendDenominations)
        {
            BOOST_FOREACH(const COutput& out, vCoins)
            {
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, std::vector<COutput>& vCoinsRet, CAmount& nValueRet, int nSpySendRoundsMin, int nSpySendRoundsMax)
{
    vecTxInRet.clear();
    vCoinsRet.clear();
    nValueRet = 0;
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            int nRounds = GetInputSpySendRounds(txin);
            if(nRounds >= nSpySendRoundsMax) continue;
            if(nRounds < nSpySendRoundsMin) continue;

            BOOST_FOREACH(int nBit, vecBits) {
                if(out.tx->vout[out.i].nValue == vecSpySendDenominations[nBit]) {
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(wtx.vout[i].nValue <= vecSpySendDenominations.back()/10) continue;
                // ignore anonymized
                if(GetInputSpySendRounds(CTxIn(wtx.GetHash(), i)) >= nSpySendRounds) continue;
            }
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nSpySendRoundsMin, int nSpySendRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, false, nSpySendRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT1000IFMN : ONLY_DENOMINATED);
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    if (!GetCollateralTxIn(txinCollateral, nValue)) {
        strReason = "SpySend requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
...
bool CWallet::GetEvolutionSystemCollateralTX(CTransaction& tx, uint256 hash, CAmount amount, bool fUseInstantSend)
{
    CWalletTx wtx;
    if(GetEvolutionSystemCollateralTX(wtx, hash, amount, fUseInstantSend)){
        tx = (CTransaction)wtx;
        return true;
    }
    return false;
...
bool CWallet::GetEvolutionSystemCollateralTX(CWalletTx& tx, uint256 hash, CAmount amount, bool fUseInstantSend)
{
    // make our change address
    CReserveKey reservekey(this);
    ...
    if(!success){
        LogPrintf("CWallet::GetEvolutionSystemCollateralTX -- Error: %s\n", strFail);
        return false;
    }
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstantSend))
                {
                    if (nCoinType == ONLY_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 ARC.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough SpySend non-denominated funds for this transaction that are not equal 1000 ARC.");
                    } else if (nCoinType == ONLY_DENOMINATED) {
                        strFailReason = _("Unable to locate enough SpySend denominated funds for this transaction.");
                        strFailReason += " " + _("SpySend uses exact denominated amounts to send funds, you might simply need to anonymize some more coins.");
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-triad-address
                        CScript scriptChange;
