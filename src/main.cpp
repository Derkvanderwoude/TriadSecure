...
#if defined(NDEBUG)
# error "Triad Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight ==0) {
        nSubsidyBase = 100100100; //100,100,100.00 Premine
		LogPrintf("First block: Transfer members balance %i TRI\n", nSubsidyBase);
		/*
		/	Trident Premine,
		/		The trident premine is created in public and clearly labled in the code base
		/		for the trident wallet. The funds from this first block will be used as follows
		/
		/		Coin Swap for XXX,XXX,XXX
		/		Development Fund = 
		/		Promotionl Materials
		/		Services and Fees (Sundrys)
		/
		/
		*/
    } else if(nPrevHeight <= 250)
	{
		...
		LogPrintf("First block: Transfer members balance %i TRI\n", nSubsidyBase);
		/*
		/	In order to bring diff up just before launch and secure the premine in the chain for 
		/   smooth execution, BLOCK 1 contains the premine, the next 249 blocks
		/	have a coinbase reward of 0, allowing for the launch team to mine with reasonable power
		/	and bring the diff up of the lowest possible setting, helping to keep the coin stable
		/	at launch and prevent the network from over producing at launch
		*/
	}		
	else{
        // GPU/ASIC mining era
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // TRI : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(TRI): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(TRI): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // TRI : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(TRI): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(TRI): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        Triad Related Inventory Messages
