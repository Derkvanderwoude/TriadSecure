...
UniValue gobject(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in triad.conf\n"
    ...
    */
    ...
    if(strCommand == "prepare")
    {
        ...
        CWalletTx wtx;
        if(!pwalletMain->GetEvolutionSystemCollateralTX(wtx, govobj.GetHash(), govobj.GetMinCollateralFee(), false)) {
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("triad.conf", statusObj));
    ...
    if(strCommand == "vote-alias")
    {
        ...
        BOOST_FOREACH(CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries())
        {
            ...
            // SETRIH FOR THIS MASTERNODE ON THE NETWORK, THE NODE MUST BE ACTIVE TO VOTE
...
UniValue getsuperblockbudget(const UniValue& params, bool fHelp)
{
    ...
    CAmount nEvolution = CSuperblock::GetPaymentsLimit(nBlockHeight);
    std::string strEvolution = FormatMoney(nEvolution);
