...
int nSpySendRounds = DEFAULT_PRIVATESEND_ROUNDS;
int nSpySendAmount = DEFAULT_PRIVATESEND_AMOUNT;
...
{
    if(fLiteMode) return; // ignore all Triad related functionality
    ...
    if(strCommand == NetMsgType::DSACCEPT) {
        ...
        if(dsq.fReady) {
            ...
            if(nState == POOL_STATE_QUEUE) {
                LogPrint("privatesend", "DSQUEUE -- SpySend queue (%s) is ready on masternode %s\n", dsq.ToString(), pmn->addr.ToString());
                SubmitDenominate();
            }
        } else {
            BOOST_FOREACH(CSpySendQueue q, vecSpySendQueue) {
            ...
            LogPrint("privatesend", "DSQUEUE -- new SpySend queue (%s) from masternode %s\n", dsq.ToString(), pmn->addr.ToString());
        ...
        if(nSessionID != nMsgSessionID) {
            LogPrint("privatesend", "DSFINALTX -- message doesn't match current SpySend session: nSessionID: %d  nMsgSessionID: %d\n", nSessionID, nMsgSessionID);
            return;
        }
        ...
        if(nSessionID != nMsgSessionID) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current SpySend session: nSessionID: %d  nMsgSessionID: %d\n", darkSendPool.nSessionID, nMsgSessionID);
            return;
        }
...
void CSpySendPool::InitDenominations()
{
    ...
    */
	vecSpySendDenominations.push_back( (500       * COIN)+500000 );
	vecSpySendDenominations.push_back( (100       * COIN)+100000 );
...
void CSpySendPool::CommitFinalTransaction()
{
    ...
    if(!mapSpySendBroadcastTxes.count(hashTx)) {
        CSpySendBroadcastTx dstx(finalTransaction, activeMasternode.vin, GetAdjustedTime());
        dstx.Sign();
...
/*
    ...
    allow endless transaction that would bloat Triad and make it unusable. To
...
void CSpySendPool::CheckForCompleteQueue()
{
    ...
    if(nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        CSpySendQueue dsq(nSessionDenom, activeMasternode.vin, GetTime(), true);
...
{
    if(fMasterNode) {
        LogPrintf("CSpySendPool::SendDenominate -- SpySend from a Masternode is not supported currently.\n");
        return false;
    }
    ...
    if(!nSessionID) {
        LogPrintf("CSpySendPool::SendDenominate -- No Masternode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return false;
    }
...
bool CSpySendPool::CheckPoolStateUpdate(PoolState nStateNew, int nEntriesCountNew, PoolStatusUpdate nStatusUpdate, PoolMessage nMessageID, int nSessionIDNew)
{
    ...
    if(nStatusUpdate == STATUS_REJECTED) {
        LogPrintf("CSpySendPool::CheckPoolStateUpdate -- entry is rejected by Masternode\n");
        UnlockCoins();
        SetNull();
        SetState(POOL_STATE_ERROR);
...
bool CSpySendPool::SignFinalTransaction(const CTransaction& finalTransactionNew, CNode* pnode)
{
    ...
    LogPrintf("CSpySendPool::SignFinalTransaction -- pushing sigs to the masternode, finalMutableTransaction=%s", finalMutableTransaction.ToString());
...
bool CSpySendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if(mnodeman.size() == 0) {
        LogPrint("privatesend", "CSpySendPool::DoAutomaticDenominating -- No Masternodes detected\n");
    ...
    if(nLiquidityProvider || fUseQueue) {
        ...
        BOOST_FOREACH(CSpySendQueue& dsq, vecSpySendQueue) {
            ...
            CMasternode* pmn = mnodeman.Find(dsq.vin);
            if(pmn == NULL) {
                LogPrintf("CSpySendPool::DoAutomaticDenominating -- dsq masternode is not in masternode list, masternode=%s\n", dsq.vin.prevout.ToStringShort());
                continue;
            }
            ...
            LogPrintf("CSpySendPool::DoAutomaticDenominating -- attempt to connect to masternode from queue, addr=%s\n", pmn->addr.ToString());
    ...
    while(nTries < 10) {
        ...
        if(pmn == NULL) {
            LogPrintf("CSpySendPool::DoAutomaticDenominating -- Can't find random masternode!\n");
        ...
        if(pmn->nLastDsq != 0 && pmn->nLastDsq + nMnCountEnabled/5 > mnodeman.nDsqCount) {
            LogPrintf("CSpySendPool::DoAutomaticDenominating -- Too early to mix on this masternode!"
        ...
        LogPrintf("CSpySendPool::DoAutomaticDenominating -- attempt %d connection to Masternode %s\n", nTries, pmn->addr.ToString());
        CNode* pnode = (pnodeFound && pnodeFound->fMasternode) ? pnodeFound : ConnectNode((CAddress)pmn->addr, NULL, true);
        if(pnode) {
...
bool CSpySendPool::CreateNewSession(int nDenom, CTransaction txCollateral, PoolMessage& nMessageIDRet)
{
    ...
    if(!fUnitTest) {
        ...
        CSpySendQueue dsq(nDenom, activeMasternode.vin, GetTime(), false);
...
bool CSpySendQueue::CheckSignature(const CPubKey& pubKeyMasternode)
{
    std::string strMessage = vin.ToString() + boost::lexical_cast<std::string>(nDenom) + boost::lexical_cast<std::string>(nTime) + boost::lexical_cast<std::string>(fReady);
    std::string strError = "";
...
bool CSpySendBroadcastTx::CheckSignature(const CPubKey& pubKeyMasternode)
{
    std::string strMessage = tx.GetHash().ToString() + boost::lexical_cast<std::string>(sigTime);
    std::string strError = "";
...
void CSpySendPool::SetState(PoolState nStateNew)
{
    if(fMasterNode && (nStateNew == POOL_STATE_ERROR || nStateNew == POOL_STATE_SUCCESS)) {
        LogPrint("privatesend", "CSpySendPool::SetState -- Can't set state to ERROR or SUCCESS as a Masternode. \n");
        return;
    }
...
{
    if(fLiteMode) return; // disable all Triad specific functionality
    ...
    RenameThread("triad-privatesend");
