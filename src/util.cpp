...
const char * const BITCOIN_CONF_FILENAME = "triad.conf";
const char * const BITCOIN_PID_FILENAME = "triadd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "triad" is a composite category enabling all Triad-related debug output
            if(ptrCategory->count(string("triad"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TriadCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TriadCore
    // Mac: ~/Library/Application Support/TriadCore
    // Unix: ~/.triadcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TriadCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TriadCore";
#else
    // Unix
    return pathRet / ".triadcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty triad.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override triad.conf
